...
imgBackground = cv2.imread("resources/bgg.png")
...
while True:
    ...
    if faceCurFrame:
        for encodeFce, faceLoc in zip(encodeCurFrame, faceCurFrame):
            ...
            faceDis = face_recognition.face_distance(encodeListKnown, encodeFce)
        ...
        if counter != 0:
            ...
            if counter == 1:
                ...
                dateTimeObject = datetime.strptime(studentInfo['last_attendance_time'],
                                                   "%Y-%m-%d %H:%M:%S")
                secondsGone = (datetime.now() - dateTimeObject).total_seconds()
                print(secondsGone)
            ...
            if modeType != 3:
                ...
                if counter <= 10:
                    ...
                    cv2.putText(imgBackground, str(id), (1006, 510),
                                cv2.FONT_HERSHEY_PLAIN, 2, (50, 50, 50), 2)

                    cv2.putText(imgBackground, str(studentInfo['major']), (1006, 570),
                                cv2.FONT_HERSHEY_PLAIN, 2, (50, 50, 50), 2)

                    cv2.putText(imgBackground, str(studentInfo['joining_year']), (1006, 630),
                                cv2.FONT_HERSHEY_PLAIN, 2, (50, 50, 50), 2)
